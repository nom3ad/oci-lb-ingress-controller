...
type NetworkingInterface interface {
	...
	ListSubnets(ctx context.Context, compartmentId string, vcnId string) ([]core.Subnet, error)
}
...
func (c *client) ListSubnets(ctx context.Context, compartmentId string, vcnId string) ([]core.Subnet, error) {
	if !c.rateLimiter.Reader.TryAccept() {
		return nil, RateLimitError(false, "GetSubnet")
	}

	resp, err := c.network.ListSubnets(ctx, core.ListSubnetsRequest{
