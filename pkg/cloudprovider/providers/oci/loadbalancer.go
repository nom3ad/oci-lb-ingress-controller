...
const (
	...
	lbDefaultShape = "10Mbps" //! replaced "100Mbps"

	lbNodesHealthCheckPath  = "/healthz"
	lbNodesHealthCheckPort  = 10256 //! replaced: k8sports.ProxyHealthzPort
...
// 	if s, ok :=  GetAnnotation(svc,ServiceAnnotationLoadBalancerSubnet1); ok && len(s) != 0 {
// 		subnets[0] = s
...
// 	if s, ok :=  GetAnnotation(svc,ServiceAnnotationLoadBalancerSubnet2); ok && len(s) != 0 {
// 		r, err := cp.client.Networking().IsRegionalSubnet(ctx, s)
// 		if err != nil {
// 			return nil, err
// 		}
	...
	for _, action := range actions {
		...
		case *ListenerAction:
			...
			if a.Type() == Delete {
				...
				// to fetch it from the load balancer
