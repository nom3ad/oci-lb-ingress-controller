...
// ReadConfig consumes the config Reader and constructs a Config
...
func NewConfigurationProvider(cfg *Config) (common.ConfigurationProvider, error) {
	...
	if cfg != nil {
		...
		if os.Getenv("OCI_CONFIG_FILE_PROFILE") != "" {
			configFilePath := os.Getenv("OCI_CONFIG_FILE")
			if configFilePath == "" {
				configFilePath = "~/.oci/config"
			}
			cp, err := common.ConfigurationProviderFromFileWithProfile(configFilePath, os.Getenv("OCI_CONFIG_FILE_PROFILE"), "")
			if err != nil {
				return nil, errors.Wrap(err, "failed to get ConfigurationProviderFromFile")
			}
			return cp, nil
		}
