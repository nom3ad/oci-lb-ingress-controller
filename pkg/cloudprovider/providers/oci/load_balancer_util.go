...
func (b *BackendSetAction) Entity() string {
	return "BackendSet"
}
...
func (b *ListenerAction) Entity() string {
	return "Listener"
}
...
func hasListenerChanged(logger *zap.SugaredLogger, actual loadbalancer.Listener, desired loadbalancer.ListenerDetails) bool {
	...
	if fmt.Sprint(actual.HostnameNames) != fmt.Sprint(desired.HostnameNames) {
		listenerChanges = append(listenerChanges, fmt.Sprintf(changeFmtStr, "Listener:HostnameNames", fmt.Sprint(actual.HostnameNames), fmt.Sprint(desired.HostnameNames)))
	}
	if toString(actual.RoutingPolicyName) != toString(desired.RoutingPolicyName) {
		listenerChanges = append(listenerChanges, fmt.Sprintf(changeFmtStr, "Listener:RoutingPolicyName", toString(actual.RoutingPolicyName), toString(desired.RoutingPolicyName)))
	}
	if fmt.Sprint(actual.RuleSetNames) != fmt.Sprint(desired.RuleSetNames) {
		listenerChanges = append(listenerChanges, fmt.Sprintf(changeFmtStr, "Listener:RuleSetNames", fmt.Sprint(actual.RuleSetNames), fmt.Sprint(desired.RuleSetNames)))
	}
	///////////////////
...
// 	ports := []int{}
// 	annotation, ok :=  GetAnnotation(svc,ServiceAnnotationLoadBalancerSSLPorts)
// 	if !ok || annotation == "" {
// 		return ports, nil
// 	}
...
func actionIndex(action Action) int {
	var actionOrder = []string{"BackendSet:create", "BackendSet:update", "Listener:create", "Listener:update", "Listener:delete", "BackendSet:delete"}
	for i := range actionOrder {
		if actionOrder[i] == action.Entity()+":"+string(action.Type()) {
			return i
		}
	}
	return -1
...
func sortAndCombineActions(logger *zap.SugaredLogger, backendSetActions []Action, listenerActions []Action) []Action {
	...
	sort.SliceStable(actions, func(i, j int) bool {
		a1 := actions[i]
		a2 := actions[j]
		return actionIndex(a1) < actionIndex(a2)
		// xxxx // Sort by the name until we get to the point a1 and a2 are Actions upon
		// xxxx // an associated Listener and BackendSet (which share the same name).
		// xxxx // if getSanitizedName(a1.Name()) != getSanitizedName(a2.Name()) {
		// xxxx 	// 	return getSanitizedName(a1.Name()) < getSanitizedName(a2.Name())
		// xxxx // }
		// xxxx
		// xxxx // For Create and Delete (which is what we really care about) the
		// xxxx // ActionType will always be the same so we can get away with just
		// xxxx // checking the type of the first action.
		// xxxx // switch a1.Type() {
		// xxxx // case Create:
		// xxxx // 	// Create the BackendSet then Listener.
		// xxxx // 	_, ok := a1.(*BackendSetAction)
		// xxxx // 	return ok
		// xxxx // case Update:
		// xxxx // 	// Doesn't matter.
		// xxxx // 	return true
		// xxxx // case Delete:
		// xxxx // 	// Delete the Listener then BackendSet.
		// xxxx // 	_, ok := a2.(*BackendSetAction)
		// xxxx // 	return ok
		// xxxx // default:
		// xxxx // 	// Should never be reachable.
		// xxxx // 	logger.Errorf("Unknown action type received: %+v", a1)
		// xxxx // 	return true
		// xxxx // }
	})
	return actions
